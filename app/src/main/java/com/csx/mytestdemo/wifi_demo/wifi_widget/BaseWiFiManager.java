...
public class BaseWiFiManager {
    ...
    public List<WifiMessageBean> getWifiInfoLists() {
        if (null != mWifiManager) {
            List<WifiMessageBean> wifiMessageBeanList = new ArrayList<>();
            for (ScanResult scanResult : mWifiManager.getScanResults()) {
                WifiMessageBean wifiMessageBean = new WifiMessageBean();
                wifiMessageBean.setSecurity_type(getSecurityMode(scanResult));
                wifiMessageBean.setScanResult(scanResult);
                wifiMessageBean.setLevel(calculateSignalLevel(scanResult.level));
                wifiMessageBeanList.add(wifiMessageBean);
            }
            return wifiMessageBeanList;
        }
        return null;
    }
