...
public class WiFiManager extends BaseWiFiManager {
     ...
     * @return 配置是否成功
     */
    public boolean connectWEPNetwork(@NonNull ScanResult result, @NonNull String password) {

        // 获取networkId
        int networkId = setWEPNetwork(result.SSID, password);
        if (-1 != networkId) {
            // 保存配置
            boolean isSave = saveConfiguration();
            // 连接网络
     ...
     * @return 配置是否成功
     */
    public boolean connectWPA2Network(@NonNull ScanResult result, @NonNull String password) {
    ...
    public int isWifiConfig(ScanResult scanResult) {
        ...
        if (getConfiguredNetworks() == null || 0 == getConfiguredNetworks().size()) {
            return -1;
        }
        List<WifiConfiguration> configWifiList = getConfiguredNetworks();
        for (WifiConfiguration configuration : configWifiList) {
            if (configuration.SSID.equals("\"" + ssid + "\"")) {

                //在这里，直接使用 networkId 进行连接
                int networkId = configuration.networkId;

                boolean isSucceedConnectedSaveWifi = mWiFiManager.enableNetwork(networkId);
                Log.d(TAG, "isWifiConfig: 连接已保存的wifi："+isSucceedConnectedSaveWifi);
